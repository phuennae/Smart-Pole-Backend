            ...
            <div class="col">
                <div class="card camera-card shadow-sm">
                    <div class="video-wrapper">
                        ...
                        <?php 
                        $url = $cam['ip_address'];
                        // ตรวจสอบประเภท URL เพื่อเลือกวิธีแสดงผล
                        if (strpos($url, 'stream.html') !== false || strpos($url, 'bmatraffic.com') !== false): 
                        ?>
                            <iframe src="<?= htmlspecialchars($url) ?>" allow="autoplay; fullscreen"></iframe>
                        
                        <?php elseif (strpos($url, 'rtsp://') === 0): ?>
                            <div class="video-placeholder bg-dark text-white text-center w-100 h-100">
                                ...
                                <p class="small">RTSP Stream<br>คลิกเปิดในโปรแกรมภายนอก</p>
                            </div>
                        ...
                        <div class="cam-controls">
                            <a href="<?= htmlspecialchars($url) ?>" target="_blank" class="btn btn-dark btn-cam border-light shadow">
    ...
    <div class="modal-dialog">
        <form action="process_camera.php?action=add" method="POST" class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="mb-3"><label>URL/IP (HTTP หรือ RTSP)</label><input type="text" name="ip" class="form-control" required></div>
    ...
    <div class="modal-dialog">
        <form action="process_camera.php?action=edit" method="POST" class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="mb-3"><label>URL/IP</label><input type="text" name="ip" id="edit_ip" class="form-control" required></div>
...
// Auto Refresh สำหรับกรณีกล้องส่งเป็นภาพนิ่ง (Snapshot)
setInterval(function(){
    document.querySelectorAll('img[id^="cam-img-"]').forEach(img => {
        if(img.src.indexOf('http') === 0 && img.src.indexOf('stream.html') === -1) {
