...
<head>
    ...
    <style>
        ...
        .ptz-btn:active { background: rgba(13,110,253,0.9); transform: scale(0.95); }
        .ptz-btn.center { background: rgba(220,53,69,0.7); }
        .ptz-btn.center:hover { background: rgba(220,53,69,0.9); }
        .ptz-status {
            font-size: 0.65rem;
            color: #adb5bd;
            text-align: center;
                    ...
                    <div class="card camera-card shadow-sm">
                        <div class="video-wrapper">
                            ...
                            <div class="ptz-panel" id="ptz-panel-<?= $cam['id'] ?>" 
                                ...
                                <div class="ptz-grid">
                                    <div></div>
                                    <button class="ptz-btn" onmousedown="ptzMove(<?= $cam['id'] ?>, 'up')" onmouseup="ptzStop(<?= $cam['id'] ?>)" onmouseleave="ptzStop(<?= $cam['id'] ?>)" title="ขึ้น">
                                        <i class="fas fa-arrow-up"></i>
                                    </button>
                                    <div></div>
                                    <button class="ptz-btn" onmousedown="ptzMove(<?= $cam['id'] ?>, 'left')" onmouseup="ptzStop(<?= $cam['id'] ?>)" onmouseleave="ptzStop(<?= $cam['id'] ?>)" title="ซ้าย">
                                        <i class="fas fa-arrow-left"></i>
                                    </button>
                                    <button class="ptz-btn center" onclick="ptzStop(<?= $cam['id'] ?>)" title="หยุด">
                                        <i class="fas fa-stop"></i>
                                    </button>
                                    <button class="ptz-btn" onmousedown="ptzMove(<?= $cam['id'] ?>, 'right')" onmouseup="ptzStop(<?= $cam['id'] ?>)" onmouseleave="ptzStop(<?= $cam['id'] ?>)" title="ขวา">
                                        <i class="fas fa-arrow-right"></i>
                                    </button>
                                    <div></div>
                                    <button class="ptz-btn" onmousedown="ptzMove(<?= $cam['id'] ?>, 'down')" onmouseup="ptzStop(<?= $cam['id'] ?>)" onmouseleave="ptzStop(<?= $cam['id'] ?>)" title="ลง">
...
function stopPlayback() {
    ...
    document.querySelector('.playback-video').innerHTML = '<canvas id="playbackCanvas"></canvas>';
...
function downloadRecording() {
    ...
    const url = `playback_proxy.php?action=download&camera_id=${cameraId}&start=${encodeURIComponent(start)}&end=${encodeURIComponent(end)}`;
    
    window.open(url, '_blank');
    debugLog(`⬇️ Downloading: ${start} → ${end}`, 'info');
}
