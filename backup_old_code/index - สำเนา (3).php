                                        ...
                                        <?php if ($files && is_array($files)): ?>
                                            <div class="d-flex flex-column gap-2">
                                                <select class="form-select form-select-sm" id="file_<?= $node['id'] ?>">
                                                    ...
                                                    foreach ($files as $f): 
                                                        $fname = $f['name'];
                                                        // --- ซ่อนไฟล์ alarm007.mp3 ---
                                                        if (strtolower($fname) === 'alarm007.mp3') continue;
...
// ฟังก์ชันส่งสัญญาณเตือนภัย (มี Confirm)
function sendAlarmBroadcast() {
    if (confirm("⚠️ ยืนยันการส่งสัญญาณเตือนภัย?\n\nระบบจะสั่งเล่นไฟล์ 'alarm007.mp3' ไปยังทุกโหนดทันที!")) {
        submitBroadcast('play', '/alarm007.mp3');
    }
}

// ฟังก์ชันหยุดทุกอย่างทันที (ไม่ต้อง Confirm)
function stopAllBroadcast() {
    submitBroadcast('stop', '');
}

// ฟังก์ชันกลางสำหรับส่งค่าไปยัง process_broadcast.php
...
function updateStatuses() {
    ...
    fetch('get_node_status.php?id=<?= $node['id'] ?>')
        ...
        .catch(err => {
            document.getElementById('online-<?= $node['id'] ?>').innerHTML = '<span class="badge bg-danger">Error</span>';
        });
    <?php endforeach; ?>
}

updateStatuses();
setInterval(updateStatuses, 5000); // อัปเดตทุก 5 วินาที
</script>
</body>
</html>