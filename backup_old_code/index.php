    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content border-0 shadow">
            ...
            <div class="modal-body">
					<div class="row g-2 mb-3 p-3 bg-light rounded align-items-center">
                    <div class="col-md-2">
                        <select id="sch_day" class="form-select form-select-sm" title="วัน">
                    ...
                    <div class="col-md-2">
                        <select id="sch_hr" class="form-select form-select-sm" title="ชั่วโมง">
                            <?php for($i=0;$i<24;$i++) printf("<option value='%d'>%02d</option>", $i, $i); ?>
                        </select>
                    </div>
                    <div class="col-md-2">
                        <select id="sch_mn" class="form-select form-select-sm" title="นาที">
                            <?php for($i=0;$i<60;$i++) printf("<option value='%d'>%02d</option>", $i, $i); ?>
                        </select>
                    </div>
                    <div class="col-md-3">
                        <select id="sch_file" class="form-select form-select-sm" title="ไฟล์เพลง"><option value="">เลือกไฟล์...</option></select>
                    </div>
                    <div class="col-md-2">
                        <div class="input-group input-group-sm">
                            <input type="number" id="sch_vol" class="form-control" placeholder="ความดัง" min="0" max="100" value="50" title="ระดับเสียง (0-100%)">
                            <span class="input-group-text">%</span>
                        </div>
                    </div>
                    <div class="col-md-1">
                        <button class="btn btn-success btn-sm w-100" onclick="addSchedule()" title="เพิ่มตารางเวลา">
...
function addSchedule() {
    const volValue = document.getElementById('sch_vol').value;
    ...
    const data = {
        ...
        volume: volValue // <--- เพิ่มการส่งค่าระดับเสียงตรงนี้
    };
    
    if(!data.file) return alert("กรุณาเลือกไฟล์");
    if(volValue === '' || volValue < 0 || volValue > 100) return alert("กรุณาระบุระดับเสียงให้ถูกต้อง (0-100)");

    fetch('proxy_scheduler.php', { 
        method: 'POST', 
...
function uploadToNode(id, ip, port) {
    ...
    const fileManager = document.getElementById('file-manager-' + id);

    if (!fileInput.files[0]) return alert("เลือกไฟล์ก่อน");

    // ซ่อนช่องอัปโหลด และแสดงสัญลักษณ์กำลังอัปโหลด
    area.style.display = 'none';
    loading.style.display = 'block';
    // แสดงสถานะกำลังโหลดในช่องรายชื่อไฟล์ด้วย
    fileManager.innerHTML = '<div class="spinner-border spinner-border-sm text-primary opacity-50"></div>';
    ...
    xhr.onload = function() {
        ...
        if (xhr.status === 200 && xhr.responseText.trim() === 'success') {
            // ไม่ต้องแสดง Alert กวนใจ แต่ให้หน่วงเวลา 1 วินาทีแล้วรีเฟรชรายชื่อไฟล์เฉพาะโหนดนี้
            setTimeout(() => {
                fetch(`get_node_files.php?id=${id}&ip=${ip}&port=${port}`)
                    .then(res => res.text())
                    .then(html => { 
            ...
            alert("เกิดข้อผิดพลาดในการอัปโหลด: " + xhr.responseText);
            loadAllNodeFiles(); // กรณีพลาดให้โหลดใหม่ทั้งหมด
        }
    };
    ...
    xhr.onerror = function() {
        loading.style.display = 'none';
        area.style.display = 'block';
        alert("การเชื่อมต่อกับเซิร์ฟเวอร์ผิดพลาด");
        loadAllNodeFiles();
    };
