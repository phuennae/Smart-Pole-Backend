    ...
    <div class="welcome-card">
        <div class="row align-items-center">
            <div class="col-md-8">
                ...
                <p class="lead mb-0">เลือกโหนดที่ต้องการเพื่อสั่งการพร้อมกันได้ทันที</p>
            </div>
        </div>
    </div>
    ...
    <div class="row mb-4 g-3">
        ...
        <div class="col-6 col-md-3">
            <button onclick="stopAllBroadcast()" class="btn p-0 w-100 text-start border-0">
                <div class="stat-card">
                    <div class="icon-box bg-dark text-white"><i class="fas fa-hand-paper"></i></div>
...
async function bulkAlarm() {
    ...
    formData.append('action', 'play_selected');
    formData.append('nodes', JSON.stringify(nodes));
    formData.append('filename', '/alarm007.mp3'); // ชื่อไฟล์ Alarm มาตรฐาน
...
async function bulkStop() {
    ...
    const formData = new URLSearchParams();
    formData.append('action', 'stop_selected'); // ใช้ action เดียวกับที่สร้างไว้ตอน Live
    formData.append('nodes', JSON.stringify(nodes));
...
function sendAlarmBroadcast() { if(confirm("ยิงเตือนทุกเครื่อง?")) submitBroadcast('play', '/alarm007.mp3'); }
function stopAllBroadcast() { if(confirm("หยุดทุกเครื่อง?")) submitBroadcast('stop', ''); }
