...
<head>
    ...
    <style>
        ...
        /* สไตล์ Volume Slider */
        .vol-control { margin-top: 10px; padding-top: 10px; border-top: 1px dashed #e2e8f0; cursor: default; }
    ...
    <div class="row justify-content-center">
        <div class="col-lg-7">
            ...
            <div id="nodeList">
                ...
                <div class="node-item shadow-sm" onclick="toggleNodeSelect(this, <?= $node['id'] ?>)">
                    ...
                    <div class="vol-control d-flex align-items-center" onclick="event.stopPropagation();">
                        <i class="fas fa-volume-down text-muted me-2 small"></i>
                        <input type="range" class="form-range flex-grow-1 mx-2" 
                               min="0" max="100" value="70" 
                               oninput="updateVolLabel(this)" 
                               onchange="setNodeVolume('<?= htmlspecialchars($node['ip_address']) ?>', this.value)">
                        <i class="fas fa-volume-up text-muted ms-2 me-3 small"></i>
                        <span class="badge bg-light text-primary border vol-label" style="width: 50px;">70%</span>
                    </div>
...
// === ฟังก์ชันจัดการ Volume (แบบ 0-100%) ===
function updateVolLabel(slider) {
    // แสดงตัวเลข % แบบ Real-time ขณะลาก
    slider.parentElement.querySelector('.vol-label').innerText = slider.value + '%';
}

function setNodeVolume(ip, volPercent) {
    // แปลงจาก 0-100% กลับเป็นสเกล 0-21 สำหรับให้บอร์ด ESP32 เข้าใจ
    let espVol = Math.round((volPercent * 21) / 100);
    
    console.log(`Setting volume for ${ip} to ${volPercent}% (ESP32 Scale: ${espVol})`);
    
    // ตั้งค่าระดับเสียงที่บอร์ด
    fetch(`http://${ip}/vol?v=${espVol}`, { mode: 'no-cors' })
        .then(() => {
            // สั่งให้บอร์ดเซฟค่าระดับเสียง
            fetch(`http://${ip}/savevol`, { mode: 'no-cors' });
        })
        .catch(err => console.error(`Volume API error on ${ip}:`, err));
}
// =====================================
...
async function startLive() {
    // 1. ดึงข้อมูลโหนดที่ถูกติ๊กเลือก
    ...
    try {
        ...
        // 2. ปรับการดึงข้อมูล activeNodes ให้เก็บค่า volume ของแต่ละเครื่องมาด้วย
        activeNodes = selectedNodes.map(n => {
            // ค้นหา slider (input range) ที่อยู่ภายใน card ของโหนดนี้
            const slider = n.closest('.node-item').querySelector('input[type="range"]');
            const volPercent = slider ? slider.value : 70;
            return {
                ip: n.dataset.ip, 
                port: n.dataset.port,
                vol: volPercent
        ...
        // (ตัวเลือกเสริม: ยิงคำสั่ง Set Volume ของแต่ละโหนดซ้ำอีกรอบทันทีก่อนเริ่มสตรีม)
        activeNodes.forEach(node => {
            setNodeVolume(node.ip, node.vol);
        });

        // 3. สั่งโหนดปลายทางให้เตรียมรับ Stream
